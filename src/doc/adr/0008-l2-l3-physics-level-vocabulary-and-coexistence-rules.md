...
## Update (2026-08-31) -- re-validated on Isaac Sim 6.0.1 + five API/behaviour findings

Empirically re-validated on Isaac Sim 6.0.1 / Isaac Lab 3.0 (driver 610) after the 5.1.0 -> 6.0.1 migration (#250). Reproducible drivers committed under `src/script/exp_l2_*.py` (isaac#193/#201/#217/#197/#228). The L2 kinematic **zero-error hard guarantee holds** (max tracking drift ~6e-8 m / ~0 deg under gravity + external contact -- float32 read-back floor, unchanged from the 5.1 `0.0000` baseline). Five 6.0.1-specific findings refine the rules above:

1. **`set_world_pose` is a `setGlobalPose` teleport (clause 3, sharpened).** On 6.0.1 `SingleRigidPrim.set_world_pose` moves a kinematic body but leaves its solver velocity at **0**, so it does NOT drive velocity-dependent contact effects (friction carry, momentum disturbance) -- a payload resting on it does not move at any friction. The true kinematic target that computes solver velocity is `world.physics_sim_view.create_rigid_body_view(path).set_kinematic_targets(data, indices)` (pos xyz + quat **xyzw**, warp float32, device cpu). `set_linear_velocity` on a kinematic body is silently ignored. This generalises the #215 lesson: **every** pose-teleport path (dc `set_rigid_body_pose`, `set_world_pose`, USD `xformOp` writes) bypasses the contact integrator for velocity; only `set_kinematic_targets` is a true kinematic drive. Pure position tracking and normal-overlap push are fine with `set_world_pose`.

2. **The literal "articulation welded to a kinematic anchor via a maximal loop joint" (clause 4) hard-crashes on 6.0.1.** A standalone floating articulation (`ArticulationRootAPI`) joined to a kinematic anchor by a maximal `UsdPhysics` loop joint reproducibly **native-SIGSEGVs the PhysX tensor backend** (`libomni.physx.tensors`, exit 139) at physics-view init -- no Python exception. The supported hybrid seam on 6.0.1 is therefore a **plain dynamic rigid body** (no `ArticulationRootAPI`) joined to the kinematic anchor. Tracked for upstream. Clause 4's "standalone L3 articulation ... connected via a loop joint" is currently **not usable** as literally stated on 6.0.1; use a dynamic rigid body until the tensor-backend crash is fixed upstream.

3. **The maximal loop `FixedJoint` is effectively rigid, not compliant.** Contrary to the PhysX-#308 "weak/soft maximal fixed joint" expectation, on 6.0.1 the seam give stays at the float32 floor (~2.4e-8 m static, ~3.6e-7 m peak) and is **mass-invariant from 1 kg to 1000 kg** with perfect force transfer (`follow_ratio` 0.9999999, no blow-up). Genuine boundary compliance requires a **softened D6 / spring joint**, not a `FixedJoint`.

4. **USD-hierarchy parenting does NOT carry a floating articulation.** A floating multibody lives in the world inertial frame, so making it a USD child of a moving kinematic prim does not drag it along (ride-along error = the full base travel). **Base carry requires a joint** (a maximal revolute/fixed joint to a dynamic link) -- confirming clause 4's reasoning that kinematic transport is a joint relationship, not a scene-graph one. Measured base carry via a revolute-maximal joint is rigid (`follow_ratio` 1.0), with base-motion disturbance on held arm joints scaling linearly with base acceleration (0.5 deg @ 2 m/s^2, 1.26 deg @ 5 m/s^2) and settling back to ~1e-6 rad residual.

5. **Tensor-view aliasing.** Reading a kinematic prim via `SingleRigidPrim` while a `create_rigid_body_view` is held on the same prim invalidates the tensor view ("prim deleted while being used by a shape in a tensor view"); read all bodies through the tensor views' `get_transforms` instead.
