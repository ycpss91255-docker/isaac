...
> **Editorial note (2026-09-07):** The `./run.sh` / `./exec.sh` wrapper commands referenced
> elsewhere predate the justfile migration. Current equivalents: `./run.sh` -> `just docker run`,
> `./exec.sh` -> `just docker exec`. Decision content is unchanged.
