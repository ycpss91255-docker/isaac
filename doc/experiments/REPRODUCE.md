...
`init_isaac_dirs.sh` MUST run before `make build`; skipping it lets the docker
daemon create the cache mounts as root and Isaac Sim then fails to write.

### 1.2 Generate `.env.generated` + `compose.yaml`

`make setup -- apply` regenerates `.env.generated` (compose variable cache) and
`compose.yaml` (the profile-gated services: `devel`, `test`, `headless`,
`stream`, `producer`) from `.setup.conf` + system detection. `apply` is the
default subcommand, so a bare `make setup` does the same. `make build` /
`make run` auto-regenerate on drift, so an explicit apply is only needed for a
scripted/CI path or after editing `.setup.conf`.

```bash
make setup -- apply           # regenerate .env.generated + compose.yaml
```
...
```bash
make run -- -t stream -d                                   # idle container + WebRTC streaming
make exec -- -t stream /isaac-sim/python.sh <driver>       # launch the driver into it
make stop                                                  # cleanup
```
...
- Browser: the `omniverse_web_viewer` sidecar (bundled at `web_viewer/`). After
  ...
  `config/host.yaml`, `make run -- -t stream -d` brings up the viewer; open
...
| A-227-multijoint (#227) | `exp_multijoint_sag.py` | `/isaac-sim/python.sh .../exp_multijoint_sag.py --out .../test/.multijoint-sag.json` | tip total ~60.2 mm (measured 60.3), additive over 3 links (30.3/20.2/9.8 base->tip); cross-joint transient 39.2 mm -> residual ~0.019 mm | Serial position-drive sag compounds linearly (Sum mg/k); held joints deviate then re-settle | numbers only |
| A-216-tracking (#216) | `exp_traj_tracking.py` | `/isaac-sim/python.sh .../exp_traj_tracking.py --out .../test/.traj-tracking.json` | steady-state ~1.76 mm at ref stiffness (mg/k floor 1.962 mm); repeat spread 0 (deterministic) | L3 tracking sits on the analytic steady-state floor; runs are reproducible across resets | numbers only |
...
| C-221-seam (#221) | `exp_l2_loop_joint_boundary.py` | `/isaac-sim/python.sh .../exp_l2_loop_joint_boundary.py --out .../test/.l2-loopjoint.json` | dynamic-body ("rigid") seam give ~10 um @ 10 kg, follow_ratio 1.000000; give rises with load/chain. `artic` variant SIGSEGVs (exit 139, #803) -- crashes the tensor backend | Maximal-coordinate fixed joint transmits force (follow ~1) and is compliant (give > 0, load-dependent); single mid-load seam is near-rigid | numbers only (`rigid` default; `--variants artic` reproduces the crash) |
...
```bash
make run -- -t stream -d
make exec -- -t stream /isaac-sim/python.sh /home/<user>/work/src/script/exp_l25_dynamic_interaction.py --mode viz --out /home/<user>/work/test/.l25-viz-trace.json
```
