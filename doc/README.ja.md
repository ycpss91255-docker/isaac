...
Isaac Sim 5.1 は Humble と Jazzy 両方の内部 ROS 2 ライブラリを `/isaac-sim/exts/isaacsim.ros2.bridge/{humble,jazzy}/` に同梱。本イメージは **Humble に固定**し、CoreSAM スタックの他の部分や `ros1_bridge`（Noetic ↔ Humble）との互換性を維持。同梱ライブラリは Python 3.11 の rclpy で、kit 内蔵インタプリタと一致。

bridge extension `isaacsim.ros2.bridge` はデフォルトの kit experience（`isaacsim.exp.full.kit` → `isaac.startup.ros_bridge_extension`）経由で自動 load。`runheadless.sh` / `runapp.sh` に `--enable` flag は不要。

env 配線は `setup.conf [environment]` に記述（全項目は CHANGELOG 参照）：

| 変数 | 値 | 理由 |
|------|-----|------|
| `ROS_DISTRO` | `humble` | 24.04 の `setup_ros_env.sh` が自動デフォルトする jazzy を上書き |
| `RMW_IMPLEMENTATION` | `rmw_fastrtps_cpp` | 明示的に FastDDS 指定、host から継承しない |
| `LD_LIBRARY_PATH` | `/isaac-sim/exts/isaacsim.ros2.bridge/humble/lib` | 同梱の FastDDS / msg type support `.so` ライブラリ |
| `FASTRTPS_DEFAULT_PROFILES_FILE` | `/isaac-sim/fastdds.xml` | UDPv4 のみの profile（コンテナ間 DDS が安定） |
| `ROS_DOMAIN_ID` | `0` | 小規模チームのデフォルト；同一 host を複数ユーザーで共有する場合は変更 |

> 通常 `/isaac-sim/setup_ros_env.sh` が `LD_LIBRARY_PATH` を自動設定するにもかかわらず、ここでは明示的に設定する必要がある：このヘルパーは lib-path の更新を `if [ -z "$ROS_DISTRO" ]` の中に包んでいる。jazzy デフォルトを上書きするため事前に `ROS_DISTRO=humble` を export しているので、ヘルパーは short-circuit して lib-path 更新をスキップしてしまう。
