...
## ROS 2 bridge（内蔵、distro-agnostic）

Isaac Sim 5.1 は Humble と Jazzy 両方の内部 ROS 2 ライブラリを `/isaac-sim/exts/isaacsim.ros2.bridge/{humble,jazzy}/` に同梱しており、両方とも kit 内蔵インタプリタと一致する Python 3.11 rclpy。**本イメージは distro 選択をオープンに保つ** — `ROS_DISTRO` と `LD_LIBRARY_PATH` は `setup.conf` で意図的に未設定とし、Isaac の `/isaac-sim/setup_ros_env.sh` に起動時に自動検出させる：

| イメージ Ubuntu | 自動 distro |
|---|---|
| 22.04 (jammy) | humble |
| 24.04 (noble) — 本イメージ | **jazzy** |
...
`setup.conf [environment]` で配線済みの env：
