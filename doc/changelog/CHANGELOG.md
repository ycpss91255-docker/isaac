...
- Override to humble at compose run time when needed: `-e ROS_DISTRO=humble -e LD_LIBRARY_PATH=/isaac-sim/exts/isaacsim.ros2.bridge/humble/lib`. Both are required together — `setup_ros_env.sh` wraps the lib-path update inside `if [ -z "$ROS_DISTRO" ]`, so once `ROS_DISTRO` is preset the helper skips priming `LD_LIBRARY_PATH`.
