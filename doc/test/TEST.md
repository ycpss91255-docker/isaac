...
| `test_build_scene_adapter.py` (3) | MR gate (#152, ADR-0018 decisions 1/3): the example scene spawns via the framework `build_scene` adapter (`to_isaaclab_cfg` -> `sim_utils` cfg -> `cfg.func()`), independently of the example driver's own raw-pxr `_build_scene` (which still runs until #154). A module-scoped fixture runs `_build_scene_runner.py` once (one `SimulationApp`) and shares its marker stdout. `test_adapter_runner_clean_exit` (`[EXIT CLEAN]`, no `[RAISED]`, returncode 0); `test_adapter_spawns_ground_light_robot` (`[ADAPTER OK] ground=True light=True robot=True objects>=1` -- environment ground (a procedural collision cuboid, NOT the nucleus `GroundPlaneCfg`) + light + robot USD authored on the live stage); `test_adapter_spawns_mobility_dynamic_object` (a `mobility: dynamic` object materializes a `UsdFileCfg` with `rigid_props`/`mass_props`/`collision_props` and `cfg.func` spawns it). `base_link` presence and the dynamic object's `RigidBodyAPI` are reported, not asserted: the committed `camera_bot.usd` / `prop_cube.usda` are legacy-importer assets with no `defaultPrim`, so the `UsdFileCfg` references resolve empty (the physics API lands on absent referenced content) until #154 regenerates the example assets with a `defaultPrim`. Marker-line acceptance; headless; runner `_build_scene_runner.py` (underscore-prefixed, not collected). |
