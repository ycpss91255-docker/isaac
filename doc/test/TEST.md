...
| `test_import_model.py` | 28 | `isaac_devkit.model_import` (former `import_model.py`): single-USD CLI plumbing (ADR-0018 decision 6) -- path resolution to one `<name>.usd`, the single-USD existing-file check, output-dir creation, `package://` URDF preprocessing, the `import_urdf` URDF-missing precondition, plus the ADR-0020 decision 6 preprocess extensions -- xacro detection + standalone expansion (#169, `pytest.importorskip("xacro")`; well-formed plain URDF, no leftover `xacro:`/`${...}`, missing-package raise-path) and the best-effort meters unit sanity check (#170, warns on a likely-mm / large `<mesh scale>` export, silent on meters / malformed XML). |
