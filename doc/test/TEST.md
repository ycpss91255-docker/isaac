...
| `test_isaaclab_available.py` (1) | MR gate (#149, ADR-0018): the baked Isaac Lab base tool is importable inside the GPU container. `test_isaaclab_importable_in_container` launches `AppLauncher` headless, imports `isaaclab.sim`, and asserts the spawner surface (`UsdFileCfg` / `GroundPlaneCfg`), the `isaaclab.sim.converters.UrdfConverterCfg` surface, and the pinned `2.3.x` version (Dockerfile `ISAACLAB_VERSION`). Pass criterion = `[ISAACLAB OK] version=... spawn=True urdf_converter=True` + `[EXIT CLEAN]` marker lines (Kit `_exit(0)`). Runner `_isaaclab_available_runner.py` (underscore-prefixed, not collected). Headless, no cameras. The runtime spawn / driver imports are exercised in full by MR-2..MR-6. |
